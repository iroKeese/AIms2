...
\\\vspace{.25cm}
 \begin{small}
  ...
  \vspace{.03cm}\\
\vspace{.6cm}
 {This version: 14$^{\text{th}}$ April, 2026}
\vspace{.6cm}
\end{small}
\end{center}
...
\begin{abstract}
  ...
  All in all, our results are consistent with the idea that preference-matching can raise initial hesitation and heighten agency-related concerns for a subset of users, functioning as an acceptance barrier even when personalization is normatively appealing.
...
Our work sits at the intersection of literatures on (i) the behavioral uptake of algorithmic decision aids, (ii) delegation, agency, and responsibility in human--algorithm interaction, (iii) transparency as an interpretive cue for acceptance, (iv) personalization as a design feature of algorithmic decision rules, and (v) heterogeneity in trust and delegation. Below, we situate our study within these streams, identify the specific gap our experiment addresses---the missing comparison between personalized and objective decision rules---and connect them to the robo-advisor application domain.
...
\subsection{The missing comparison: personalized versus objective decision rules}
...
Within the preference-based condition, however, never-delegators reported higher satisfaction with their Part-1 choices than delegators (7.94 vs. 7.56; $t = -1.400$, $p = 0.169$), which is inconsistent with the interpretation that non-delegation mainly reflects dissatisfaction with the elicited preference profile used by the personalized algorithm.
...
To characterize feedback dynamics, we estimate a win-stay/lose-shift model on rounds 2--10. Under a preference-based algorithm, a prior win (a payoff larger than zero) is associated with a higher—though imprecisely estimated—probability of staying with the same delegation/non-delegation choice (\(\beta_{\text{lagWin}}\) = 0.043, SE = 0.033, $p = 0.194$). The interaction between the EV treatment and a prior win is negative and significant ($\beta$ = -0.102, SE = 0.049, $p = 0.041$), implying that in the EV condition, participants tend to shift their delegation/non-delegation decision after a non-0 payoff \textit{more} often than after a zero-payoff (approximately 0.043 - 0.102 = -0.059; still, even in this case, stay rates are 60\%; also note that average stay rates after losses are 69\% in the Preference-based condition and 65\% in the EV condition, so that we generally do not observe a win-stay--lose-shift pattern).
...
Additional reason items are informative by contrast. Items such as not wanting to be analyzed by an algorithm were rarely selected overall, suggesting that profiling concerns or dehumanization were unlikely to be primary drivers of treatment differences in this setting. By comparison, selecting the task being tedious appears more among delegators (though still at a low level, 10.9\% vs. 0\%), consistent with an effort-saving channel among participants who are willing to hand over control. Open-text responses also repeatedly mention time pressure and limited time to scan all options; these comments point to a general choice-friction channel, but they do not isolate a treatment-specific mechanism.

These mechanism patterns should be interpreted cautiously. The reason measures are ex-post and refer to hypothetical additional choices rather than the incentivized rounds themselves. They therefore function as suggestive proxies for motives, not as causal identification of psychological channels or as decisive evidence for one mechanism over another.
...
\caption{Condition-specific marginal effects on delegation probability. The plot shows all regression coefficients from the linear probability model (main effects and treatment interactions). Key expectation variables include the number of non-preferred choices (the participant's own prediction of how many tasks, out of 10, the algorithm would choose differently from their preference) and the number of completely weird choices (predicted count of highly unusual choices). The preference-based-condition estimate is the main-effect coefficient; the EV-condition estimate is the sum of the main effect and the treatment interaction term. Error bars are 95\% confidence intervals based on participant-clustered standard errors. The model controls for decision round, per-choice Part-1 confidence, revealed Part-1 risk preferences (measured by the number of choices that were less risky in the sense of having fewer states with a payoff of 0), and task fixed effects.}
