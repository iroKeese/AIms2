# Script to create all 5 tables for the paper
# Run this script to generate publication-ready tables
...
table1_latex <- table1_data %>%
  kable(
    ...
    col.names = c("Algorithm", "N", "Age (M)", "Age (SD)", "Female (\\%)", "University Education (\\%)"),
    format = "latex",
    digits = 2,
    booktabs = TRUE,
    escape = FALSE
...
# Save as LaTeX
writeLines(as.character(table1_latex), "Table1_DescriptiveStats.tex")

cat("Table 1 saved as:\n")
cat("  - Table1_DescriptiveStats.csv\n")
cat("  - Table1_DescriptiveStats.tex\n\n")
...
table2_data <- data.frame(
  Variable = c("EV Algorithm", "Decision Number", "EV Algorithm $\\times$ Decision Number"),
...
table2_latex <- table2_data %>%
  kable(
    ...
    col.names = c("Variable", "Estimate", "SE", "p-value", "Estimate", "SE", "p-value"),
    format = "latex",
    digits = 4,
    align = c("l", rep("c", 6)),
    booktabs = TRUE,
  ...
  add_header_above(c(" " = 1, "All Periods" = 3, "Periods 2-10" = 3)) %>%
  kable_styling(
    latex_options = c("HOLD_position", "striped"),
    full_width = FALSE
  )
...
# Save as LaTeX
writeLines(as.character(table2_latex), "Table2_RegressionResults.tex")

cat("Table 2 saved as:\n")
cat("  - Table2_RegressionResults.csv\n")
cat("  - Table2_RegressionResults.tex\n\n")
...
# Create LaTeX table
table3_latex <- table3_data %>%
  kable(
    caption = "Never-Delegator Characteristics",
    col.names = c("Group", "N", "Age (M)", "Age (SD)", "Female (\\%)", "Absurd Computer Choices (M)", "Absurd Computer Choices (SD)"),
    format = "latex",
    digits = 2,
    booktabs = TRUE,
    escape = FALSE
...
# Save as LaTeX
writeLines(as.character(table3_latex), "Table3_NeverDelegators.tex")

cat("Table 3 saved as:\n")
cat("  - Table3_NeverDelegators.csv\n")
cat("  - Table3_NeverDelegators.tex\n\n")
...
table4_data <- data.frame(
  Variable = c("EV Algorithm", "Previous Win", "EV Algorithm $\\times$ Previous Win"),
  Estimate = c(
...
# Save as LaTeX
writeLines(as.character(table4_latex), "Table4_WinStayLoseShift.tex")

cat("Table 4 saved as:\n")
cat("  - Table4_WinStayLoseShift.csv\n")
cat("  - Table4_WinStayLoseShift.tex\n\n")
...
# Create LaTeX table
table5_latex <- table5_data %>%
  kable(
    caption = "Correlations Between Delegation and Individual Characteristics",
    col.names = c("Delegation", "Age", "Female", "Education", "Bad Computer Choices", "Absurd Computer Choices", "Risk Taking"),
    format = "latex",
    digits = 3,
    booktabs = TRUE
  ) %>%
...
# Save as LaTeX
writeLines(as.character(table5_latex), "Table5_Correlations.tex")

cat("Table 5 saved as:\n")
cat("  - Table5_Correlations.csv\n")
cat("  - Table5_Correlations.tex\n\n")

cat("\n===========================================\n")
cat("ALL TABLES CREATED SUCCESSFULLY\n")
cat("===========================================\n")
cat("Files saved:\n")
cat("  - Table1_DescriptiveStats.csv/tex\n")
cat("  - Table2_RegressionResults.csv/tex\n")
cat("  - Table3_NeverDelegators.csv/tex\n")
cat("  - Table4_WinStayLoseShift.csv/tex\n")
cat("  - Table5_Correlations.csv/tex\n")