...
\title[Title]{\textbf{When personalization deters delegation to
		...
		experiment}}
\author[Suraci \& Wolff\hspace{1.3cm}]{Lara Suraci\,\textsuperscript{a} \& Irenaeus Wolff\,\textsuperscript{b}\\\vspace{.2cm}\footnotesize \textsuperscript{a}DG Cities \quad \textsuperscript{b}Thurgau Institute of Economics (TWI) / University of Konstanz}
\date[Wolff @ AI WS 2026]{AI Workshop (Berlin), 23$^{\text{rd}}$ June, 2026}


\begin{document}
\logo{\includegraphics[scale = .37]{logo_twi_cmyk.pdf}\vspace{220pt} \hspace{-.15cm}}
% THIS IS THE TITLEFRAME
\begin{frame}
	\vspace{3.6cm}
	{\LARGE \twi{When personalization deters delegation to algorithms:}}\\\vspace{.15cm}
	{\large \twi{Evidence from a \textbf{preregistered} experiment}}\\
\vspace{.5cm}
{\large Lara Suraci\;\&\;Irenaeus Wolff}\vspace{.1cm}\\
{\small  --- 23$^\text{rd}$ June, 2026, Symposium on `AI and the Future of Work', Berlin ---}\\\vspace{.2cm}
...
\logo{\includegraphics[scale = 0.25]{logo_twi_cmyk.pdf}\vspace{225pt} \hspace{-.12cm}}


\frame{\frametitle{Background}
 \begin{itemize}
  \item Huge \& unconnected literature on algorithm/automation/AI reliance ($\rightarrow$ review by Chugunova \& Sele, 2022)
  \item Common assumption: \twi{\textbf{personalisation}} (tailoring to the user's preferences) should \twi{increase} acceptance of an algorithmic decision aid
  \item But personalisation may also be \twi{double-edged}: a system that ``decides as you would'' substitutes for personal judgment more directly than an impersonal rule
  \item[\twi{$\rightarrow$}] Does that backfire? Does personalisation actually raise \textit{behavioural} willingness to delegate?
 \end{itemize}
}
...
\frame{\frametitle{Relevance?}
 ...
 \end{itemize}
  ...
  \frame{\frametitle{Research Question}
   ...
   \begin{frame}
 	\frametitle{Objective vs Personalised Algorithms}
 	\begin{itemize}
 	\setlength\itemsep{1em}
   \item What do we mean by \textcolor{lightblue}{\textbf{objective optimization}}?
   \begin{itemize}
   ...
   \end{itemize}
   \item What do we mean by \textcolor{lightblue}{\textbf{personalised}}?
   \begin{itemize}
   ...
   \end{itemize}
   \item[\twi{$\triangleright$}] ``Objective'' is a \textit{label}, not a normative claim: we are not asserting that EV-maximisation is rational, nor that revealed preferences are perfectly measured.
  \end{itemize}
\end{frame}
   ...
   \textcolor{lightblue}{\textbf{Prefs}}: \textit{`The algorithm analyses your choices from Part 1 and selects the best option for you in light of your Part-1 choices.'}
    ...
    \item[\textcolor{lightblue}{\textbf{Part 2}}] \textcolor{lightblue}{-- } Lottery Selection Task
     \begin{itemize}
      ...
      \item each Part-2 task built from 2 \twi{anchor options} (transformed from Part 1) + 6 \twi{decoys} (each dominated by an anchor, but not obviously so)
     \end{itemize}
  	\end{itemize}
  	\end{frame}
 	...
 	\frame{\frametitle{Tasks}
 		...
 		\end{figure}
 		\begin{itemize}
 			\item 8 incentivised rounds; 4 filler tasks interleaved in Part 1
 			\item Per round: \twi{5\,s} pre-view $\rightarrow$ \twi{5\,s} delegate/not $\rightarrow$ up to \twi{8\,s} own decision\vspace{-.2cm}
 		\end{itemize}
 	}

 	\frame{\frametitle{Sample, Incentives \& Procedure}
 		\begin{itemize}
 			\item \twi{Recruitment:} Prolific UK, online (z-Tree unleashed); preregistered target 120/treatment before exclusions.
 			\item \twi{Exclusions (preregistered):} bot/slider checks, comprehension after 2nd attempt, self-reported AI use, implausibly fast log-times, incomplete data.
 	...
 	\frame{\frametitle{Preregistered Hypothesis \& Analysis Plan}
 		\begin{itemize}
 			...
 			\begin{itemize}
 				...
 				\item[(2)] One-sided \textbf{Wilcoxon--Mann--Whitney} on participant-level mean delegation across Part~2
 			\end{itemize}
 			\item \twi{Preregistered LPM:} decision-set FE, participant-clustered SE, treatment\,$\times$\,period interaction, share of prior delegated rounds with zero payoff.
 		...
 		\begin{columns}
 			\begin{column}{0.5\textwidth}
 				\begin{itemize}
 					...
 					\item \twi{Round 1:} \textcolor{lightblue}{\textbf{66.9\% (EV)}} vs.\ \textcolor{lightblue}{\textbf{55.8\% (Prefs)}}.
 					\item Boschloo exact test, one-sided: \twi{$p = 0.0413$} $\Rightarrow$ \textbf{H1 supported at first contact.}
 					\item Treatment gap concentrated in the first decision.
 				\end{itemize}
 			\end{column}
 			\begin{column}{0.5\textwidth}
 				\begin{figure}
 	...
 	\frame{\frametitle{Mechanisms: \textit{What} deters delegation differs by algorithm}
 		\begin{columns}
 			\begin{column}{0.55\textwidth}
 				\begin{itemize}
 					\item LPM of individual delegation on \textit{expected non-preferred} and \textit{expected ``completely weird''} choices, $\times$ treatment, controlling for round, Part-1 confidence, risk preferences, task FE:
 			...
 			\begin{column}{0.45\textwidth}
 				\begin{figure}
 					\includegraphics[width=\textwidth]{CoefPlotBadAbsurd.pdf}
 				\end{figure}
 			\end{column}
 		\end{columns}
 	}
 	...
 	\frame{\frametitle{Dynamics: No clean win-stay/lose-shift\;\;\small{(secondary)}}
 		\begin{columns}
 			\begin{column}{0.5\textwidth}
 				\begin{itemize}
 			...
 			\begin{column}{0.5\textwidth}
 				\begin{figure}
 					\includegraphics[width=\textwidth]{Figure5_WinStayLoseShift.pdf}
 				\end{figure}
 			\end{column}
 		\end{columns}
 	}
